# Hunter Outreach Guide

## Was ist ein Hunter?

Ein "Hunter" auf Product Hunt ist jemand, der Produkte einreicht. Wenn ein bekannter Hunter mit vielen Followern dein Produkt "hunted", bekommst du automatisch Visibility bei seinen Followern.

**Vorteile eines guten Hunters:**

- Ihre Follower werden über den Launch benachrichtigt
- Mehr initiale Upvotes
- Höhere Credibility
- Bessere Chance auf "Featured" Status

---

## Top Hunter kontaktieren

### Tier 1: Super-Influencer (10k+ Follower)

Diese sind schwer zu erreichen, aber der Versuch lohnt sich:

| Hunter          | Follower | Fokus             | Kontakt                   |
| --------------- | -------- | ----------------- | ------------------------- |
| Chris Messina   | 40k+     | Tech General      | @chrismessina (Twitter)   |
| Mubashar Iqbal  | 35k+     | SaaS, Tools       | @mubashariqbal (Twitter)  |
| Marc Köhlbrugge | 15k+     | Indie Makers      | @marckohlbrugge (Twitter) |
| Fabian Siegel   | 12k+     | European Startups | @fabiansiegel (Twitter)   |

### Tier 2: Aktive Hunter (1-10k Follower)

Realistischere Targets:

| Hunter              | Follower | Fokus           |
| ------------------- | -------- | --------------- |
| Kevin William David | 8k+      | AI/ML Products  |
| Arun Pariyar        | 5k+      | SaaS Tools      |
| Andreas Klinger     | 4k+      | Startup Tools   |
| Benedicte Raae      | 3k+      | Developer Tools |

### Tier 3: Nischen-Hunter (500-1k Follower)

Höchste Erfolgswahrscheinlichkeit:

Suche auf Product Hunt nach Huntern, die ähnliche Produkte gelauncht haben:

- Review Management Tools
- AI Writing Tools
- Chrome Extensions
- Small Business Software

---

## Outreach Templates

### Twitter DM (Kurz)

```
Hey [Name]! 👋

I'm launching ReviewResponder on Product Hunt - an AI tool that helps businesses respond to customer reviews in seconds.

Would you be interested in hunting it? I'd love to have someone with your expertise introduce it to the PH community.

Happy to share a demo or answer any questions!

Best,
Berend
```

### Email (Ausführlicher)

```
Subject: Product Hunt Launch - ReviewResponder (AI Review Responses)

Hi [Name],

I've been following your Product Hunt activity and noticed you've hunted several AI/SaaS products. I'm reaching out because I think ReviewResponder might be a great fit for your portfolio.

**What is ReviewResponder?**
An AI-powered tool that generates professional, personalized responses to customer reviews in seconds. Any language with auto-detect. Plus a Chrome Extension that works directly on Google Maps.

**Why it's interesting:**
- Solves a real pain point for local businesses
- Uses GPT-4o-mini for high-quality responses
- 4 different tones (Professional, Friendly, Formal, Apologetic)
- Chrome Extension for one-click responses

**Launch details:**
- Target date: [DATE]
- Exclusive 60% launch discount
- Demo video and all assets ready

Would you be interested in hunting ReviewResponder? I'd be happy to:
- Give you a full demo
- Provide early access
- Answer any questions

Thanks for considering!

Best,
Berend Mainz
ReviewResponder

P.S. Here's a quick preview: https://review-responder-frontend.onrender.com
```

### LinkedIn Message

```
Hi [Name],

I came across your Product Hunt profile and was impressed by your track record of launching innovative products.

I'm preparing to launch ReviewResponder - an AI tool that helps businesses respond to customer reviews in seconds. Given your experience in the [AI/SaaS/Productivity] space, I thought you might be interested in hunting it.

Would you be open to a quick 15-min call to discuss?

Best,
Berend
```

---

## Outreach Timeline

### 3-4 Wochen vor Launch

- Liste der Target-Hunter erstellen
- Social Media Profile der Hunter recherchieren
- Ihre bisherigen Hunts analysieren

### 2-3 Wochen vor Launch

- Erste Kontaktaufnahme (Twitter/LinkedIn)
- Personalisierte Nachrichten senden
- Product Demo anbieten

### 1 Woche vor Launch

- Follow-up bei Interessierten
- Finale Zusagen einholen
- Alternative Hunter als Backup

### Falls kein Hunter gefunden

**Kein Problem!** Du kannst dein eigenes Produkt "hunten" (als Maker).

- Fokussiere dich stattdessen auf Community-Upvotes
- Baue dein eigenes Hunter-Profil auf für zukünftige Launches

---

## Do's and Don'ts

### Do's ✅

- Personalisiere jede Nachricht
- Recherchiere den Hunter vorher
- Biete Mehrwert (früher Zugang, Feedback)
- Sei professionell und freundlich
- Follow-up nach 5-7 Tagen

### Don'ts ❌

- Keine Massen-DMs
- Nicht verzweifelt wirken
- Keinen Druck ausüben
- Nicht zu früh nachfragen
- Keine falschen Versprechungen

---

## Tracking Template

| Hunter | Plattform  | Gesendet | Follow-up | Status       |
| ------ | ---------- | -------- | --------- | ------------ |
| [Name] | Twitter DM | [Datum]  | [Datum]   | ⏳ Warten    |
| [Name] | Email      | [Datum]  | -         | ✅ Zugesagt  |
| [Name] | LinkedIn   | [Datum]  | [Datum]   | ❌ Abgelehnt |

---

## Notizen

- Beste Zeit für DMs: Werktags 9-17 Uhr (Zeitzone des Hunters beachten!)
- Antwortrate liegt typischerweise bei 10-20%
- Plan für mind. 10 Outreaches, um 1-2 Zusagen zu bekommen
